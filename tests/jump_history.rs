...
mod tests {
    ...
    fn test_jump_history_logic() {
        ...
        let target = state.jump_back(state.reading_state.clone()).unwrap();
        ...
        assert!(state.jump_back(state.reading_state.clone()).is_none());
        assert_eq!(state.reading_state.row, 10);
        assert_eq!(state.jump_history_index, 0);

        // 6. Jump Forward
        let target = state.jump_forward().unwrap();
        state.reading_state.row = target.row;
        assert_eq!(state.reading_state.row, 20);
        assert_eq!(state.jump_history_index, 1);
        ...
        let target = state.jump_back(state.reading_state.clone()).unwrap(); // at 20 (index 1)
        state.reading_state.row = target.row;
