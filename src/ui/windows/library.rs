...
impl LibraryWindow {
    ...
    fn render_details(
        ...
        // The full path (home abbreviated to ~), wrapped by the paragraph —
        // a truncated path cannot be used to actually find the file.
        lines.push(Line::styled(
            format!(
                "Path: {}",
                crate::library::abbreviate_home(std::path::Path::new(&entry.filepath))
            ),
