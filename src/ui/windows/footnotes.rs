...
pub struct FootnotesWindow;

impl FootnotesWindow {
    pub fn render(frame: &mut Frame, area: Rect, entries: &[LinkEntry], selected_index: usize) {
        ...
        if entries.is_empty() {
            let paragraph = Paragraph::new("No footnotes on this page")
                .style(Style::default().fg(Color::DarkGray))
                .block(Block::default().title("Footnotes").borders(Borders::ALL));
        ...
        let status_line = Paragraph::new("Enter: jump  q: close")
