...
3. **Normalized selection output** (S; split into `improvements/improvement-07.md` (yank + dictionary, shared mapping helper) and `improvement-08.md` (TTS)) — yanked text, dictionary lookups, and TTS input currently include layout artifacts (justification spaces, first-line indents). With item 0, read the clean text directly from `source_map.source_text` via the selected rows' spans instead of collapsing whitespace heuristically.
