...
1. **Layout-independent search** (was M-L; split into `improvements/improvement-09.md` and `improvement-10.md`) — ✅ complete (2026-07): regexes run once per chapter over `source_map.source_text`, so phrases survive justification, wrap boundaries, and display hyphenation. Each canonical hit owns its character-column ranges on every touched rendered row and supplies the derived render index. The full selected range receives current-hit styling, `n`/`p` count hits even when anchors share a row, manual scrolling re-synchronizes navigation, and the results window shows one unbroken source-text preview per hit. Search and visual-mode highlight ranges share character units, fixing their previous byte/character mismatch on non-ASCII lines.
2. **Whitespace-robust styling recovery** (M) — ✅ complete (2026-07), shipped as `improvements/improvement-06.md` (short-term matcher fix) and `improvement-11.md` (source-coordinate migration). Bold, italic, headings, and CSS-class emphasis are stored as canonical chapter-local source ranges and projected through `SourceMap` onto every touched rendered row. The rendered-text matcher family and emphasis-marker stripping are retired, preserving literal asterisks while making styles independent of wrapping, justification, indentation, typography spacing, Unicode byte widths, and synthetic image rows. (The link half of the original item is covered by item 0 step (d).)
...
4. **KOReader-compatible push sync — not planned.** Stock KOReader requires a CREngine-native XPointer, which `repy` cannot reliably derive from source coordinates; an invalid push could overwrite a valid bookmark. KOReader has no minimal/headless CLI for generating one, while using its desktop runtime through a custom Lua helper—or maintaining plugins, a custom server, or bundled CREngine builds—would add disproportionate cross-platform and maintenance cost. Kosync therefore remains safely pull-only.
