...
0. **Canonical source coordinates (`SourceMap`)** (L) — 🚧 in progress (2026-07). The shared machinery for items 1–3: the parser now *produces* per-chapter source spans during wrapping instead of features recovering positions by text-matching afterwards. `TextStructure.source_map` maps every wrapped row to a `[start, end)` char span of the normalized chapter source text (`annotations::normalize_text` over the pre-wrap `raw_lines`), with `offset_for_row`/`row_for_offset` projection both ways. Migration steps (each shippable; design in the step-(a) commit and plan notes):
   - (a) ✅ done (2026-07): parser emits `SourceMap`; pagebreaks anchored by offset; span-invariant and cross-width round-trip tests.
   - (b) reading position/width-rebuild on source offsets (`reading_states.source_offset`, schema v8; restore ladder falls back to `rel_pctg` then raw row for legacy databases).
   - (c) bookmarks and marks on source offsets.
   - (d) links/sections/footnotes anchored by offset at parse time (retires `find_line_by_words` and its fuzzy word-matching ladder).
1. **Layout-independent search** (was M-L, now S-M on top of item 0) — search currently regexes each rendered line, so with justification on, a multi-word pattern like `foo bar` misses the stretched `foo  bar`, and no phrase split across a wrap boundary ever matches. Fix: run the regex per chapter over `source_map.source_text` and project hits to rows with `row_for_offset` (supersedes the earlier idea of lifting `build_normalized_chapter` out of `src/annotations.rs`). Remaining real work: multi-row hits need highlight rendering and `n`/`p` navigation to accept ranges instead of single rows.
